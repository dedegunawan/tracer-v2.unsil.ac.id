...
foreach($penggunas as $pengguna) {
    $start++;
    $prodi = $pengguna->prodiObject;
    $column = array(
        $start,
        $pengguna->nama_lembaga,
        $pengguna->jabatan,
        $pengguna->email_lembaga,
        $pengguna->nama_alumni,
        $pengguna->tahun_lulus,
        $pengguna->tahun_angkatan,
        @$prodi->Nama." (".@$prodi->jenjang->Nama.")",
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('Export Data Mahasiswa');
...
header('Content-Disposition: attachment;filename="data-pengguna-'.$ci->uri->segment(3).'-'.$ci->uri->segment(4).'.xls"');
