...
$pdf->Cell('180', 6, 'REKAP SARAN & KRITIK ALUMNI', 0, 1, 'C', 0);
...
foreach ($listProdi as $ProdiID) {
	...
	foreach ($datas as $d) {
		$start++;
		$data[] = array($start, ucwords(strtolower($d->nama_depan." ".$d->nama_belakang)), @$d->jawaban_alumni_temp()->where('pertanyaan_id', 11)->first()->jawaban);
	}
